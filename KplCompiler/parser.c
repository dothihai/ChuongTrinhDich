...
void compileAssignSt(void) {
  ...
  Type *lvalueType = NULL; //khai bao de bat kieu du lieu cua nhung cai doc dk
  Type *expType = NULL;
  Type **vArr = malloc(100 * sizeof(vArr));// khai bao mang de luu kieu du lieu cua 2 ben
  Type **eArr = malloc(100 * sizeof(eArr));
  int countVariables = 1; //bien dem so luong 2 ben
  int countExpression = 0;
  lvalueType = compileLValue(); // lay kieu du lieu cua phan tu dau tien
  vArr[0] = lvalueType; // dua kieu du lieu vao mang
  while(lookAhead->tokenType == SB_COMMA){ // doc lan luot cho den khi con dau phay
    eat(SB_COMMA); // an dau phay
    lvalueType = compileLValue();
    vArr[countVariables] = lvalueType;
    countVariables ++;
  }
  eat(SB_ASSIGN); // an dau gan
  ...
  if(countExpression != countVariables){ // check bieu thuc
    //error(ERR_INVALIDSTATEMENT, lookAhead->lineNo, lookAhead->colNo);
    printf("%d-%d:So luong bien va bieu thuc khac nhau", lookAhead->lineNo, lookAhead->colNo);
    return;
  }
  int i;
  for(i = 0; i < countExpression && i < countVariables; i++) // check khac kieu
    checkTypeEquality(vArr[i], eArr[i]);
}
