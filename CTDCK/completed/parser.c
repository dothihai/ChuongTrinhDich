...
void compileAssignSt(void) {
  ...
  Type *expType = NULL;
  Type **vArr = malloc(100 * sizeof(Type*));
  Type **eArr = malloc(100 * sizeof(Type*));
  int countVariables = 1;
  int countExpression = 0;
  lvalueType = compileLValue();//complieValue() tra ve kieu du lieu cua bien. o day lay bien dau tien va luu vao stack
  vArr[0] = lvalueType;//gan vao vi tri dau tien cua mang cac bien
  while(lookAhead->tokenType == SB_COMMA){
    eat(SB_COMMA);
    genINT(1);//nhay 1 o trong bo nho stack
    lvalueType = compileLValue(); //lay bien tiep theo luu vao stack
    vArr[countVariables] = lvalueType;//luu vao mang
    countVariables ++;
  }
  eat(SB_ASSIGN);
  genDCT(countVariables*2-2);//nhay o stack tu tren xuong
  expType = compileExpression();//kieu phep toan dau tien va luu vao stack
  eArr[0] = expType;//luu vao vi tri dau tien 
  countExpression++;
  while(lookAhead->tokenType == SB_COMMA){
    eat(SB_COMMA);
    genINT(1);
    expType = compileExpression();//lay phep toan tiep theo
    eArr[countExpression] = expType; 
    countExpression++;
  }
  if(countExpression != countVariables){
    //error(ERR_INVALIDSTATEMENT, lookAhead->lineNo, lookAhead->colNo);
    printf("%d-%d:So luong bien va bieu thuc khac nhau\n", lookAhead->lineNo, lookAhead->colNo);
    exit(0);
  }
  int i;
  for(i = 0; i < countExpression && i < countVariables; i++)
    {checkTypeEquality(vArr[i], eArr[i]);genST();}//lay 2 so tu dinh stack, Store s[s[t-1]] := s[t]; t := t -2;
...
Type* compileExpression(void) {
  ...
  case SB_MINUS:
    ...
    type = compileExpression2();
    checkIntType(type);
    genNEG(); //Negative s[t] := - s[t]; thuc hien phep tru, gap dau tru thi phai tru cai buu thuc do di 
    break;
  case KW_IF: 
      eat(KW_IF);
      compileCondition(); // tao khoi lenh
      eat(KW_RETURN);
      
      fjInstruction = genFJ(DC_VALUE); // fall jump vaf jump khi khoi lenh o tren sai
      
      Type* t1 = compileExpression(); // tao ra khoi lenh thu 1, dk tra ve khi dung
      
      jInstruction = genJ(DC_VALUE); // jump khi dung
      updateFJ(fjInstruction, getCurrentCodeAddress()); // sap xep lai cau jump( vi luc ban nay chua biet cx vi tri)
       //  getCurrentCodeAddress dia chi hien tai, phia sau cua jump vua tao ra
      eat(KW_ELSE); 
      eat(KW_RETURN);
      Type* t2 = compileExpression(); // xu li bieu thuc so 2

      updateJ(jInstruction, getCurrentCodeAddress()); // cap nhat lai dia chi 

      checkTypeEquality(t1, t2); // kieu tra xem t1 vs t2 co cung kieu hay ko
      // xu li dieu kien, dung fj ko dk t.h -> bthuc1 dk tra ve -> xu li
      // dkien sai fj dk t.h nhay va xu li bieu thuc so 2 bo qua bthuc 1
      type = t2;
      break;
